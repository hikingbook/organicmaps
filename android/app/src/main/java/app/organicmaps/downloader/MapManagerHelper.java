...
public class MapManagerHelper
{
    ...
    final AlertDialog dlg = new MaterialAlertDialogBuilder(context, R.style.MwmTheme_AlertDialog)
                                ...
                                .setPositiveButton(R.string.downloader_retry,
                                                   (dialog, which) -> {
                                                     ...
                                                     warn3gAndRetry(context, errorData.countryId, MapSource.ORGANIC_MAPS, listener);
                                                   })
                                .create();
  ...
  public static boolean warn3gAndDownload(Context context, final String countryId, final MapSource mapSource,
                                          @Nullable final Runnable onAcceptListener)
  {
    return warnOn3g(context, countryId, () -> {
      if (onAcceptListener != null)
        onAcceptListener.run();
      startDownload(context, countryId, mapSource);
    });
  }

  public static boolean warn3gAndRetry(Context context, final String countryId, final MapSource mapSource,
                                       @Nullable final Runnable onAcceptListener)
  {
    return warnOn3g(context, countryId, () -> {
      if (onAcceptListener != null)
        onAcceptListener.run();
      retryDownload(context, countryId, mapSource);
    });
  }
