...
struct TBatchedData
{
  ...
  storage::NodeStatus const m_newOrganicMapStatus;
  storage::NodeStatus const m_newHikingbookProMapStatus;
  storage::NodeErrorCode const m_errorCode;
  bool const m_isLeaf;

  TBatchedData(storage::CountryId const & countryId, storage::NodeStatus const newOrganicMapStatus, storage::NodeStatus const newHikingbookProMapStatus,
               storage::NodeErrorCode const errorCode, bool isLeaf)
    : m_countryId(countryId)
    , m_newOrganicMapStatus(newOrganicMapStatus)
    , m_newHikingbookProMapStatus(newHikingbookProMapStatus)
...
struct CountryItemBuilder
{
  ...
  jfieldID m_Id, m_Name, m_DirectParentId, m_TopmostParentId, m_DirectParentName, m_TopmostParentName, m_Description,
      m_Size, m_EnqueuedSize, m_TotalSize, m_ChildCount, m_TotalChildCount, m_downloadedChildCount, m_Present, m_Progress, m_DownloadedBytes,
      m_BytesToDownload, m_Category, m_Status, m_ErrorCode, m_HikingbookProMapStatus, m_HikingbookProMapSize;
...
static void UpdateItemShort(JNIEnv * env, jobject item, storage::NodeStatus const status,
                            storage::NodeErrorCode const error, storage::NodeStatus const hikingbookProMapStatus)
{
...
JNIEXPORT jint Java_app_organicmaps_sdk_downloader_MapManager_nativeGetStatus(JNIEnv * env, jclass clazz, jstring root, jint mapSourceValue)
{
  storage::NodeStatuses ns;
  GetStorage().GetNodeStatuses(jni::ToNativeString(env, root), static_cast<MapSource>(mapSourceValue), ns);
  return static_cast<jint>(ns.m_status);
}

// static void nativeGetError(String root);
JNIEXPORT jint Java_app_organicmaps_sdk_downloader_MapManager_nativeGetError(JNIEnv * env, jclass clazz, jstring root, jint mapSourceValue)
{
  storage::NodeStatuses ns;
  GetStorage().GetNodeStatuses(jni::ToNativeString(env, root), static_cast<MapSource>(mapSourceValue), ns);
...
static void EndBatchingCallbacks(JNIEnv *env) {
    ...
    for (auto const & [_, key] : g_batchedCallbackData)
    {
        ...
        for (TBatchedData const & dataItem : key.second)
        {
            ...
            SLR const item(env, env->NewObject(batchDataClass, batchDataCtor, id.get(),
...
JNIEXPORT void JNICALL Java_app_organicmaps_sdk_downloader_MapManager_nativeDownload(JNIEnv * env, jclass clazz, jstring root, jint mapSourceValue)
{
  StartBatchingCallbacks();
  GetStorage().DownloadNode(jni::ToNativeString(env, root), static_cast<MapSource>(mapSourceValue));
  EndBatchingCallbacks(env);
}

// static boolean nativeRetry(String root);
JNIEXPORT void Java_app_organicmaps_sdk_downloader_MapManager_nativeRetry(JNIEnv * env, jclass clazz, jstring root, jint mapSourceValue)
{
  StartBatchingCallbacks();
  GetStorage().RetryDownloadNode(jni::ToNativeString(env, root), static_cast<MapSource>(mapSourceValue));
  EndBatchingCallbacks(env);
}

// static void nativeUpdate(String root);
JNIEXPORT void Java_app_organicmaps_sdk_downloader_MapManager_nativeUpdate(JNIEnv * env, jclass clazz, jstring root, jint mapSourceValue)
{
  StartBatchingCallbacks();
...
{
  storage::NodeStatuses organicMapNodeStatuses;
...
// static @Nullable void nativeUpdateLocalMapRegistration(boolean isActivatedUser, boolean isBeforeProMapGracePeriod, int numDownloadedMapsLimit);
JNIEXPORT void JNICALL
Java_app_organicmaps_sdk_downloader_MapManager_nativeUpdateAllMapsRegistration(JNIEnv *env,
                                                                                jclass clazz,
                                                                                jboolean isActivatedUser,
                                                                                jboolean isBeforeProMapGracePeriod,
                                                                                jint numDownloadedMapsLimit) {
    auto &s = GetStorage();
