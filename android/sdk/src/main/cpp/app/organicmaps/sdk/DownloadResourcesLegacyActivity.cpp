...
JNIEXPORT jint JNICALL Java_app_organicmaps_sdk_DownloadResourcesLegacyActivity_nativeStartNextFileDownload(
    JNIEnv * env, jclass clazz, jint mapSourceValue, jobject listener)
{
  ...
  downloader->EnsureMetaConfigReady(mapSource, [&storage, mapSource, ptr = jni::make_global_ref(listener)]()
  {
    ...
    g_currentRequest.reset(HttpRequest::GetFile(downloader->MakeUrlListLegacy(mapSource, fileName),
