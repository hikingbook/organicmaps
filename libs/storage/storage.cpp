...
Progress Storage::GetOverallProgress(CountriesVec const & countries) const
{
  ...
  for (auto const & country : countries)
  {
    ...
    if (downloadingIt != m_downloadingCountries.cend())
    {
      ...
      overallProgress.m_bytesTotal += CalculateProgressInQueue(country);
    }
    else if (m_justDownloaded.count(country) != 0)
    {
      MapSource mapSource = MapSource::Organicmaps;
      LocalFilePtr localFile = GetLatestLocalFile(country);
      if (localFile) {
        mapSource = localFile->GetMapSource();
      }
      MwmSize const sz = GetRemoteSize(GetCountryFile(country), mapSource);
...
void Storage::SaveDownloadQueue()
{
  ...
  { ss << (ss.tellp() == 0 ? "" : ";") << country.GetCountryId() << "|" << static_cast<uint8_t>(country.GetMapSource()); });
...
void Storage::DownloadCountry(CountryId const & countryId, MapFileType type, MapSource mapSource)
{
  ...
  QueuedCountry queuedCountry(countryFile, countryId, type, m_currentVersion, m_dataDir, m_diffsDataSource, mapSource);
...
void Storage::OnMapDownloadFinished(CountryId const & countryId, DownloadStatus status, MapFileType type, MapSource mapSource)
{
  CHECK_THREAD_CHECKER(m_threadChecker, ());
...
void Storage::SetDownloadingServersForTesting(MapSource mapSource, std::vector<std::string> const & downloadingUrls)
{
...
void Storage::DownloadNode(CountryId const & countryId, MapSource mapSource, bool isUpdate /* = false */)
{
  ...
  {
    if (descendantNode.IsLeaf() && GetNodeStatus(descendantNode, mapSource).status != NodeStatus::OnDisk)
    {
    ...
    if (node.ChildrenCount() == 0) {
        switch (statusAndError.status) {
            ...
            case NodeStatus::Applying: {
                auto const mwmsInQueue = GetQueuedCountries(m_downloader->GetQueue());
                auto const downloadingProgress = CalculateProgress(node, mwmsInQueue);
                ...
                switch (mapSource) {
                    case MapSource::Organicmaps: {
                        if (proMapSizeDiff == 0 || proMapSizeDiff < organicmapSizeDiff) {
                            ...
                            LocalFilePtr const localFile = GetLatestLocalFile(node.Value().Name());
                            if (localFile && localFile->GetMapSource() == mapSource) {
                                statusAndError.status = NodeStatus::OnDisk;
                            }
                        }
                    }
                        break;
...
void Storage::ApplyDiff(CountryId const & countryId, MapSource mapSource, std::function<void(bool isSuccess)> const & fn)
{
    ...
    if (((statusAndErr.status != NodeStatus::NotDownloaded &&
        statusAndErr.status != NodeStatus::Partly) || 
         (statusAndErrForHikingbookProMaps.status != NodeStatus::NotDownloaded &&
          statusAndErrForHikingbookProMaps.status != NodeStatus::Partly)) && d.IsLeaf())
    {
...
Progress Storage::CalculateProgress(CountryTree::Node const & subtreeRoot, CountriesSet const & mwmsInQueue) const
{
  ...
  subtreeRoot.ForEachInSubtree([&](CountryTree::Node const & node)
  {
    ...
    if (downloadingIt != m_downloadingCountries.cend())
    {
      ...
      result.m_bytesTotal += CalculateProgressInQueue(d);
    }
...
int64_t Storage::CalculateProgressInQueue(CountryId const & countryId) const
{
    int64_t bytesTotal = 0;
    m_downloader->GetQueue().ForEachCountry([this, &countryId, &bytesTotal](QueuedCountry const & country)
    {
        if (country.GetCountryId() == countryId) {
            bytesTotal += GetRemoteSize(GetCountryFile(countryId), country.GetMapSource());
        }
    });
    return bytesTotal;
}
