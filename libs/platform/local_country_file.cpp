...
MapSource LocalCountryFile::GetMapSource() const
{
    if (m_countryFile.IsHikingbookProMapAvailable()) {
        ...
        auto proMapSizeDiff = std::max(size, m_countryFile.GetHikingbookProMapRemoteSize()) - std::min(size, m_countryFile.GetHikingbookProMapRemoteSize());
        auto organicmapSizeDiff = std::max(size, m_countryFile.GetRemoteSize()) - std::min(size, m_countryFile.GetRemoteSize());
